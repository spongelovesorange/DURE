...
def main():
    ...
    parser.add_argument('--data_base_path', type=str, default=None, help='数据集根目录 (默认: <repo>/datasets)')
    parser.add_argument('--log_base_path', type=str, default=None, help='日志根目录 (默认: <repo>/logs/quantization)')
    parser.add_argument('--ckpt_base_path', type=str, default=None, help='模型根目录 (默认: <repo>/ckpt/quantization)')
    parser.add_argument('--codebook_base_path', type=str, default=None, help='码本根目录 (默认: <repo>/datasets)')
    
    args = parser.parse_args()

    # --- 1. 设置路径和日志 ---
    # 解析 repo 根与 quantization 目录
    quant_dir = Path(__file__).resolve().parent
    repo_root = quant_dir.parent

    # 默认目录改为 repo-root 绝对路径，避免依赖 CWD
    if not args.data_base_path:
        args.data_base_path = str(repo_root / 'datasets')
    if not args.log_base_path:
        args.log_base_path = str(repo_root / 'logs' / 'quantization')
    if not args.ckpt_base_path:
        args.ckpt_base_path = str(repo_root / 'ckpt' / 'quantization')
    if not args.codebook_base_path:
        args.codebook_base_path = str(repo_root / 'datasets')
    try:
    ...
    if args.config_path is None:
        config_path = str(quant_dir / 'configs' / f"{args.model_name}_config.yaml")
    else:
        # 支持相对路径和绝对路径；统一转为绝对路径，基于 CWD 解析
