...
# 添加 encoder 模块所在路径
enc_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'generate_embedding')
if enc_dir not in sys.path:
    sys.path.append(enc_dir)

# 延迟导入 encoder，避免无关依赖（如 PIL、openai）阻塞离线场景
...
def main():
    ...
    parser.add_argument('--save_root', type=str, default=None, help='保存预处理数据的根目录 (默认: <repo>/datasets)')
    # ✅ (修改) image_root 指向 amazonXX/Images/ 或类似目录
    parser.add_argument('--image_root', type=str, default=None, help='包含图像信息文件和图像文件夹的根目录 (默认: <repo>/datasets)') 
    ...
    # 解析 repo 根目录 (preprocessing 的父目录)
    repo_root = Path(__file__).resolve().parents[1]
    # 规范化保存路径，使用绝对路径，避免受 CWD 影响
    if not args.save_root:
        args.save_root = str(repo_root / 'datasets')
    if not args.image_root:
        args.image_root = str(repo_root / 'datasets')
    ...
    try:
        if args.embedding_type == 'text_local':
            ...
            embeddings = _get_text_encoder().generate_local_text(args, item_text_list)
            modality_tag = "text"
            ...
            embeddings = _get_text_encoder().generate_api_text(args, item_text_list)
            modality_tag = "text"
            model_tag = args.sent_emb_model

        elif args.embedding_type == 'image_clip':
            if not image_dir: raise ValueError("图像目录未找到或无效，无法生成图像嵌入。")
            embeddings = _get_image_encoder().generate_clip_image(args, id2item, images_info, image_dir)
            modality_tag = "image"
            model_tag = args.clip_model_name

        elif args.embedding_type == 'cf_sasrec':
            embeddings = _get_cf_encoder().train_and_extract_sasrec(args, len(id2item))
            modality_tag = "cf"
            model_tag = "sasrec"

        elif args.embedding_type == 'vlm_fused':
            if not image_dir: raise ValueError("图像目录未找到或无效，无法生成 VLM 融合嵌入。")
            embeddings = _get_vlm_encoder().generate_vlm_fused(args, id2item, text_map, images_info, image_dir)
